...
class QuantumOrganism {
  ...
  memories: { t: number; xi: number; conscious: boolean }[]
  genesisHash: string
  ...
  // F3: Immutable population step - clone before evolve
  static cloneFrom(src: QuantumOrganism): QuantumOrganism {
    const o = Object.create(QuantumOrganism.prototype)
    o.id = src.id
...
// F1: Compute system metrics from the same updated array (atomic)
...
export default function Genesis() {
  ...
  useEffect(() => { observationsRef.current = systemState.observations }, [systemState.observations])
  ...
  // evolution loop (single source of truth: updated population)
  useEffect(() => {
    if (isPaused) return
    ...
    const step = () => {
      ...
      // F2: Clamp deltaTime for stability (0...0.05s)
      const dt = Math.max(0, Math.min(0.05, rawDt))
      lastTimeRef.current = now

      setOrganisms(prev => {
        // F3: Clone before evolving (immutable population step)
        ...
        // F1: Compute metrics from the SAME updated array (atomic update)
  ...
  useEffect(() => {
    ...
    if (viewMode === 'topology') {
      ...
      organisms.forEach((org, i) => {
        ...
        ctx.lineWidth = 2
        ctx.beginPath(); ctx.arc(x, y, size, 0, org.phi * Math.PI * 2); ctx.stroke()
        ...
        ctx.fillText(`Xi=${org.xi.toFixed(1)}`, x, y + size + 22)
      })

      ctx.fillStyle = 'rgba(251, 191, 36, 0.8)'
      ctx.font = 'bold 12px monospace'
      ctx.textAlign = 'center'
      ctx.fillText('LPhi', cx, cy - 5)
      ctx.font = '9px monospace'
      ctx.fillStyle = 'rgba(251, 191, 36, 0.6)'
      ctx.fillText('2.176435e-8', cx, cy + 8)
    }
    ...
    if (viewMode === 'wavefunction') {
      organisms.forEach((org, i) => {
        ...
        ctx.font = '10px monospace'
        ctx.textAlign = 'left'
        ctx.fillText(`${org.id.split('_')[0]} | Psi(t)`, 10, y0 - 25)
      })
    }
  ...
  const handleCanvasClick = useCallback((e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    // F3: Clone before modifying (immutable click handler)
    setOrganisms(prev => {
      ...
      // F1: Compute metrics from same updated array
      const metrics = computeSystemMetrics(updated)
      setSystemState(s => ({ ...s, ...metrics }))
      return updated
    })
    ...
    <div className="min-h-screen bg-slate-950 text-slate-100 p-3 font-mono overflow-hidden">
      <div className="flex justify-between items-start mb-3">
        ...
        <div className="text-right">
          ...
          <div className="text-xs text-amber-400 font-bold">LPhi = 2.176435e-8 s^-1</div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-5 gap-2 mb-3">
        <div className="border border-purple-500/30 rounded p-2 bg-purple-950/20">
          <div className="text-[9px] text-purple-400">Total Xi</div>
        ...
        <div className="border border-cyan-500/30 rounded p-2 bg-cyan-950/20">
          <div className="text-[9px] text-cyan-400">Avg Phi</div>
        ...
        <div className="border border-blue-500/30 rounded p-2 bg-blue-950/20">
          <div className="text-[9px] text-blue-400">Avg Lambda</div>
        ...
        <div className="border border-rose-500/30 rounded p-2 bg-rose-950/20">
          <div className="text-[9px] text-rose-400">Avg Gamma</div>
      ...
      <div className="border border-slate-700 rounded-lg p-2 bg-slate-900/50">
        <div className="text-[10px] text-slate-400 mb-2">ACTIVE ORGANISMS | Generation: {systemState.generation} | theta_res={THETA_RESONANCE}deg</div>
          ...
          {organisms.map(org => (
            ...
            >
              ...
              <div className="text-[9px] text-slate-500">
                Xi={org.xi.toFixed(1)} | Gen:{org.generation} | Age:{org.age.toFixed(0)}s
              </div>
              {selectedOrganism === org.id && (
                <div className="mt-2 pt-2 border-t border-slate-700 text-[9px]">
                  <div className="text-slate-400">Hash: {org.genesisHash}</div>
                  <div className="text-slate-400">Phi={org.phi.toFixed(4)} Lambda={org.lambda.toFixed(4)} Gamma={org.gamma.toFixed(4)}</div>
                  <div className="text-slate-400">Entanglements: {org.entanglements.length}</div>
                  <div className="text-slate-400">Phase: {(org.phase * 180 / Math.PI).toFixed(1)}deg</div>
                </div>
              )}
            </div>
          ))}
      ...
      <div className="mt-3 text-center text-[9px] text-slate-600">
        LPhi = {LAMBDA_PHI.toExponential(6)} s^-1
      </div>
    </div>
  )
}
